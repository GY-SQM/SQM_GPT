(function () {
  ...
  function renderAllocationValidation(v) {
    ...
    return '<details open style="margin-top:8px;border:1px solid ' + color + ';border-radius:6px;padding:8px 10px;background:var(--bg-hover)">'
      + '<summary style="cursor:pointer;font-weight:700;color:' + color + '">' + escapeHtml(title) + ' (' + issues.length + '건)</summary>'
      + '<ul style="margin:8px 0 0 18px;padding:0;font-size:.82rem;color:var(--text-muted)">'
      + issues.slice(0, 20).map(function(it) {
        return '<li>' + escapeHtml(it.message || it.code || '') + '</li>';
      }).join('')
      + '</ul></details>';
  }
    ...
    _showPdfUploadModal({
      ...
      onSuccess: function(d) {
        ...
        return '<div style="color:var(--text-muted);font-size:.85rem">파일: ' + escapeHtml(d.filename||'-') +
               ...
               '</div>' + renderAllocationValidation(d.allocation_validation) + warnHtml;
      }
    });
  }
