            ...
            with self.db.transaction():
                ...
                if tonbag_count > 0:
                    ...
                    lot_weight = self._safe_parse_float(
                        packing.get('net_weight'))
                    # [BUG-FIX #5-WEIGHT-TOLERANCE 2026-04-29]
                    # 일부 파서(BL/DO)가 net_weight에서 샘플(1kg)을 제외하므로
                    # 허용 오차를 0.5 → 0.5 + SAMPLE_WEIGHT_KG(1.0) + 0.01로 확장
                    _weight_tolerance = 0.5 + float(SAMPLE_WEIGHT_KG) + 0.01
                    if lot_weight > 0 and abs(tb_sum - lot_weight) > _weight_tolerance:
